...
class YITH_WCBK_Booking_Helper extends YITH_WCBK_Booking_Helper_Legacy {
	...
	public function count_booked_bookings_in_period( $args = array() ) {
		 ...
		 * @var bool            $exclude_reserved          True to exclude reserved bookings.
		 * @var array           $get_post_args             Get post arguments [deprecated]
		 */
		list ( $product_id, $from, $to, $count_persons_as_bookings, $exclude_order_id, $exclude, $exclude_reserved, $get_post_args ) =
			yith_plugin_fw_extract( $args, 'product_id', 'from', 'to', 'count_persons_as_bookings', 'exclude_order_id', 'exclude', 'exclude_reserved', 'get_post_args' );
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_booking_helper_count_booked_bookings_in_period_query_args
		 *
		...
		if ( ! $exclude_reserved && ! ! $product_id && ! ! $from && ! ! $to ) {
			$reserved = yith_wcbk()->reserved_bookings()->get_reserved_count( $product_id, $from, $to, $exclude_order_id, $count_persons_as_bookings );
			if ( $reserved ) {
				$count += $reserved;
			}
		}
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_booking_helper_count_booked_bookings_in_period
		 *
	...
	public function count_max_booked_bookings_per_unit_in_period( $args = array() ) {
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_count_booked_booking_in_period_args
		 *
	...
	public function get_future_bookings_by_product( $product_id, $return = 'bookings' ) {
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_get_future_bookings_by_product_args
		 *
	...
	public function get_bookings_by_order( $order_id, $order_item_id = false ) {
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_pre_get_bookings_by_order
		 *
		 * Filters the bookings retrieved for a specific order before being retrieved.
		 *
		 * @param YITH_WCBK_Booking[]|null $bookings      The bookings.
		 * @param int                      $order_id      The order ID.
		 * @param int                      $order_item_id The order item ID.
		 *
		 * @return YITH_WCBK_Booking[]|null
		 */
		$bookings = apply_filters( 'yith_wcbk_pre_get_bookings_by_order', null, $order_id, $order_item_id );
		if ( is_null( $bookings ) ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_get_bookings_by_order_args
			 *
	...
	public function get_bookings_in_time_range( $from, $to, $duration_unit = 'all', $include_externals = true, $product_id = false ) {
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_booking_helper_get_bookings_in_time_range_args
		 *
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_booking_helper_get_bookings_in_time_range
		 *
