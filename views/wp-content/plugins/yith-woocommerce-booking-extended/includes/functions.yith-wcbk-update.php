...
function yith_wcbk_update_500_global_availability_rules() {
	...
	if ( $rules ) {
		...
		foreach ( $rules as $rule ) {
			...
			$result = $wpdb->insert( $wpdb->yith_wcbk_global_availability_rules, $db_rule );
			$priority++;

			$had_error = $had_error || ! $result;
		}
	}

	if ( $had_error ) {
		update_option( 'yith_wcbk_booking_global_availability_ranges__backup', $rules, false );
	}
...
function yith_wcbk_update_500_global_price_rules() {
	...
	if ( $rules ) {
		...
		foreach ( $rules as $rule ) {
			...
			$result = $wpdb->insert( $wpdb->yith_wcbk_global_price_rules, $db_rule );
			$priority++;
	...
	if ( $had_error ) {
		update_option( 'yith_wcbk_booking_global_cost_ranges__backup', $rules, false );
	}
 ...
 * Update DB Version.
 */
function yith_wcbk_update_581_db_version() {
	YITH_WCBK_Install::update_db_version( '5.8.1' );
}
