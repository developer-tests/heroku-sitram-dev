...
if ( ! class_exists( 'YITH_WCBK_DB' ) ) {
	...
	abstract class YITH_WCBK_DB {
		...
		const RESERVED_BOOKINGS                      = 'yith_wcbk_reserved_bookings';
		const RESERVED_RESOURCES                     = 'yith_wcbk_reserved_resources';
		...
		public static function define_tables() {
			...
			$tables = array(
				...
				self::RESERVED_BOOKINGS                      => self::RESERVED_BOOKINGS,
				self::RESERVED_RESOURCES                     => self::RESERVED_RESOURCES,
			);
		...
		public static function create_db_tables() {
			...
			$reserved_bookings                      = $wpdb->prefix . self::RESERVED_BOOKINGS;
			$reserved_resources                     = $wpdb->prefix . self::RESERVED_RESOURCES;
			$collate                                = '';
			...
			$sql = "CREATE TABLE $booking_notes_table (
					...
					CREATE TABLE $reserved_resources (
					  ...
					  `timestamp` datetime NOT NULL DEFAULT '0000-00-00 00:00:00',
					  `expires` datetime NOT NULL DEFAULT '0000-00-00 00:00:00',
					  PRIMARY KEY `item_resource` (`resource_id`,`order_item_id`),
					  KEY `resource_id` (`resource_id`),
					  KEY `order_id` (`order_id`)
					) $collate;
                    ";
