		...
		*/
		...
		public function check_default_availability( int $from, int $to, array $args = array() ): bool {
			...
			if ( 'month' !== $this->get_duration_unit() && $availabilities ) {
				...
				if ( ! $is_same_day ) {
					// Check availability for each single day to allow "fluid" availability and to check each single day individually.
					$tmp_from       = $from;
					$tmp_to         = $from;
					$re_adjusted_to = ( $to + 1 );
		 ...
		 * @param bool|null $default_availability Default availability.
		 *
		 * @return bool|null
		 */
		public function check_availability_rules( int $from, int $to, bool $exclude_time = false, $default_availability = true ) {
			...
			$is_same_day = strtotime( 'midnight', $tmp_from ) === strtotime( 'midnight', $tmp_to );
			...
			if ( ! $is_same_day ) {
				// Each day needs to be checked individually!
				$tmp_from = $from;
				$tmp_to   = $from;
				...
				do {
					...
					$single_available = $this->check_availability_rules( $tmp_from, $tmp_to, $exclude_time, null );
					if ( false === $single_available || ( is_null( $single_available ) && false === $default_availability ) ) {
						return false;
					}

					if ( true === $single_available ) {
						$available = true;
					}
					$tmp_from = $tmp_to;
