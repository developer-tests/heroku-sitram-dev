...
if ( ! class_exists( 'YITH_WCBK_Cache' ) ) {
	...
	class YITH_WCBK_Cache {
		...
		public function get_object_data( $object_type, $id, $key_args = '' ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_cache_get_object_data_object_id
			 *
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_cache_get_object_data_{$object_type}_id
			 *
			 * Filters the object data ID in the internal cache.
			 *
			 * @param int          $id          The object ID.
		...
		public function delete_object_data( $object_type, $id, $key_args = '' ) {
			...
			if ( '' === $key_args ) {
				$result = $this->delete_object_transient( $object_type, $id );
			} else {
				$key_args       = $this->get_transient_key( $key_args );
				$transient_name = $this->get_transient_name( $object_type, $id );
				$transient_data = get_transient( $transient_name );
				...
				$result = set_transient( $transient_name, $transient_data, $this->transient_expiration );
			}

			/**
			 * DO_ACTION: yith_wcbk_cache_delete_{$object_type}_data
			 * Hook fired when deleting cache data for a specific object type.
			 *
			 * @param int  $id       The ID of the object.
			 * @param int  $key_args The key arguments.
			 * @param bool $result   True if the deletion succeeded; false otherwise.
			 */
			do_action( "yith_wcbk_cache_delete_{$object_type}_data", $id, $key_args, $result );

			/**
			 * DO_ACTION: yith_wcbk_cache_delete_object_data
			 ...
			 * @param bool   $result      True if the deletion succeeded; false otherwise.
			 */
			do_action( 'yith_wcbk_cache_delete_object_data', $object_type, $id, $key_args, $result );
