...
if ( ! function_exists( 'yith_wcbk_get_duration_units' ) ) {
	...
	function yith_wcbk_get_duration_units( $plural_control = 1 ) {
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_get_duration_units
		 *
		 * Filters the duration units.
		 *
		 * @param array $duration_units The duration units.
		 * @param int   $plural_control The plural control (1 for singular; 2 for plural).
		 *
		 * @return array
		 */
...
if ( ! function_exists( 'yith_wcbk_get_cancel_duration_units' ) ) {
	...
	function yith_wcbk_get_cancel_duration_units() {
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_get_cancel_duration_units
		 *
...
if ( ! function_exists( 'yith_wcbk_get_minimum_minute_increment' ) ) {
	...
	function yith_wcbk_get_minimum_minute_increment() {
		/**
		 * APPLY_FILTERS: yith_wcbk_get_minimum_minute_increment
		 *
...
if ( ! function_exists( 'yith_wcbk_get_max_months_to_load' ) ) {
	...
	function yith_wcbk_get_max_months_to_load( $unit = 'day' ) {
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_get_max_months_to_load
		 *
		 * Filters the maximum number of months to load for a specific duration unit.
		 *
		 * @param int    $months_to_load The number of months to load.
...
if ( ! function_exists( 'yith_wcbk_admin_user_info_html' ) ) {
	...
	function yith_wcbk_admin_user_info_html( $booking, $echo = true ) {
		...
		if ( $user ) {
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_admin_user_info_html
			 *
			 * Filters the HTML info for a specific user (to be shown on admin side).
			 *
			 * @param string            $html    The HTML info.
			 * @param YITH_WCBK_Booking $booking The booking.
			 * @param WP_User           $user    The user.
			 *
			 * @return string
			...
			/**
			 * APPLY_FILTERS: yith_wcbk_admin_no_user_info_html
			 *
...
if ( ! function_exists( 'yith_wcbk_get_default_colors' ) ) {
	...
	function yith_wcbk_get_default_colors(): array {
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_default_colors
		 *
		 * Filters the default plugin colors.
		 *
		 * @param array $defaults The colors.
		 *
		 * @return array
		 */
...
if ( ! function_exists( 'yith_wcbk_get_query_string_param' ) ) {
	...
	function yith_wcbk_get_query_string_param( string $key ) {
		...
		/**
		 * APPLY_FILTERS: yith_wcbk_get_query_string_param
		 *
