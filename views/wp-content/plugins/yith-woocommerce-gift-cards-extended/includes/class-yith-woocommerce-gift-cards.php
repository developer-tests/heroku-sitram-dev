		...
		public function check_gift_card( $gift, $remove = false ) {
			...
			if ( ! $remove ) {
				...
				if ( WC()->cart->total < $ywgc_minimal_car_total ) {
					...
					wc_add_notice( sprintf( __( 'In order to use the gift card, the minimum total amount in the cart has to be %s', 'yith-woocommerce-gift-cards' ), $ywgc_minimal_car_total . get_woocommerce_currency_symbol() ), 'error' );
