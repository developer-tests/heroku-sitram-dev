	...
	class YITH_YWGC_Backend {
		...
		public function enqueue_backend_files() {
			...
			if ( 'edit-gift_card' === $screen->id ) {
				...
				$last_id = $wpdb->get_var( $wpdb->prepare( "SELECT max(id) FROM {$wpdb->prefix}posts WHERE post_type = %s", YWGC_CUSTOM_POST_TYPE_NAME ) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			...
			if ( $order_gift_cards ) :
				foreach ( $order_gift_cards as $code => $amount ) :
					...
					<tr>
						...
						<td width="1%"></td>
						<td class="total"><?php echo wp_kses_post( wc_price( $amount, array( 'currency' => $currency ) ) ); ?></td>
					</tr>
					<?php
				endforeach;
			endif;
		...
		public function update_totals_on_save_order_items( $data_store, $order ) {
			...
			if ( ! $updated_as_fee && ! empty( $order_aux_total ) && $order_total !== $order_aux_total ) {
				...
				$item->set_props(
					array(
						'id'       => '_ywgc_fee',
						'name'     => sprintf( __( 'Gift Card (%s)', 'yith-woocommerce-gift-cards' ), $applied_codes_string ),
