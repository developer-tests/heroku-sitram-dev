...
if ( ! class_exists( 'YITH_YWGC_Email_Send_Gift_Card' ) ) {
	...
	class YITH_YWGC_Email_Send_Gift_Card extends YITH_YWGC_Mail {
		 ...
		 * @return bool|void
		 */
		public function trigger( $object, $case = '' ) { // phpcs:ignore Universal.NamingConventions.NoReservedKeywordParameterNames.objectFound, Universal.NamingConventions.NoReservedKeywordParameterNames.caseFound
			$result = false;
			...
			if ( is_numeric( $object ) ) {
				...
				if ( 'shop_order' === $post_type ) {
					...
					$object = new YITH_YWGC_Gift_Card_Extended( array( 'ID' => $object ) );
				} else {
					return false;
				}
			}

			if ( ! ( $object instanceof YITH_YWGC_Gift_Card_Extended ) ) {
				return false;
			}
