...
<div class="activation_window">
	...
	<div class="aw-contents-container">
		<div id="aw-activation" class="content-row <?php echo esc_attr( $show_activation ); ?>">
			...
			<div class="aw-cell">
					<label for="txt_unique_product_id"  class="aw-label">Product Id:</label>
					<select id="txt_unique_product_id"  required>
						<option value="" ></option>
						<option value="3078" ><?php esc_html_e( '3078 - Single Site', 'wf-easypost' ); ?></option>
						<option value="3079" ><?php esc_html_e( '3079 - Up to 5 Sites', 'wf-easypost' ); ?></option>
						<option value="3080" ><?php esc_html_e( '3080 - Up to 25 Sites', 'wf-easypost' ); ?></option>
					</select>
			</div>
			<div class="aw-cell">
			...
			<div class="content-row" style="float: left;">
				<p class="aw-note-cell">Check <a href="https://elextensions.com/my-account/api-keys/" target="_blank">My Account</a> for API Keys and API Downloads.</p>
			</div>
		</div>
		<div id="aw-deactivation" class="content-row <?php echo esc_attr_e( $show_deactivation ); ?>">
			...
			<div class="aw-deactivation-info">
				...
				Product ID: <span id="info-unique_product_id"><?php echo esc_attr_e( $unique_product_id_mail ); ?></span> &nbsp;|&nbsp;
...
<script>
	jQuery(document).on("click", "#btn_licence_activate",function(){
		...
		if ( licence_key.length > 0 && unique_product_id.length > 0 ) {
		...
		}else if( unique_product_id.length == 0 ){
			html_msg = "<p><strong> Product id is required to activate the licence </strong></p>";
			add_style = 'error'
			remove_style = 'updated';
			me.removeClass('deactive');
				jQuery("#result").html(html_msg)
