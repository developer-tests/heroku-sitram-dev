...
class EasypostWfTrackingUtil {
	...
	public static function convert_shipment_result_obj_to_array( $shipment_result_obj, $order_id ) {
		...
		$branding_link                    = isset( $label_settings['branding_url'] ) ? $label_settings['branding_url'] : 'no';
		if ( ! empty( $shipment_result_obj->tracking_info_obj_array ) ) {
			...
			foreach ( $shipment_result_obj->tracking_info_obj_array as $tracking_info_obj ) {
				...
				if ( ! empty( $easypost_labels ) && 'yes' === $branding_link ) {
					foreach ( $easypost_labels as $label_val ) {
						if ( $label_val['tracking_number'] === $tracking_info['tracking_id'] ) {
							$tracking_info['branding_link'] = $label_val['link'];
						}
					}
				}
		...
		if ( ! empty( $shipment_result_obj->tracking_info_api_obj_array ) ) {
			foreach ( $shipment_result_obj->tracking_info_api_obj_array as $tracking_info_api_obj ) {
				...
				if ( ! empty( $easypost_labels ) && 'yes' === $branding_link ) {
					foreach ( $easypost_labels as $label_val ) {
						if ( $label_val['tracking_number'] === $tracking_info_api['tracking_id'] ) {
							$tracking_info_api['branding_link'] = $label_val['link'];
						}
					}
				}
	...
	public static function get_shipment_custom_message( $shipment_source_data ) {
		...
		/**
		 * To add custom tracking message.
		 * 
		 * @since 1.1.0
		 */
	...
	public static function get_shipment_display_custom_message( $shipment_result_array, $shipment_source_data ) {
		...
		if ( isset( $shipment_result_array['tracking_info'] ) ) {
			...
			foreach ( $shipment_result_array['tracking_info'] as $tracking_info ) {
				...
				$tracking_link       = isset( $tracking_info['branding_link'] ) ? $tracking_info['branding_link'] : $tracking_info['tracking_link'];
				$tracking_id_substr .= ' ';
				if ( '' === $tracking_info['tracking_link'] ) {
					$tracking_id_substr .= $tracking_info['tracking_id'] . ',';
				} else {
					$tracking_id_substr .= ' <a href="' . $tracking_link . '" target="_blank">' . $tracking_info['tracking_id'] . '</a>,';
				}
			}
			...
			foreach ( $shipment_result_array['tracking_info'] as $tracking_info ) {
				if ( '' !== trim( $tracking_info['tracking_id'] ) ) {
					...
					if ( '' === $tracking_info['tracking_link'] ) {
						...
						$sub_message_2 .= ' <a href="' . $tracking_link . '" target="_blank">' . $tracking_info['tracking_id'] . '</a>,';
					}
				}
			}
	...
	public static function update_tracking_data( $order_id,
		...
		if ( ! empty( $shipment_id_cs ) ) {
			$tracking_id_trim = preg_split( '/\,/', $shipment_id_cs );
		}
		$shipment_labels = '';

		if ( isset( $shipment_source_data['shipment_id_cs'] ) ) {
			if ( ! empty( $easypost_labels ) ) {
				foreach ( $easypost_labels as $easypost_label ) {
					if ( isset( $easypost_label['tracking_number'] ) && is_array( $tracking_id_trim ) && ! empty( $tracking_id_trim ) && ! in_array( $easypost_label['tracking_number'], $tracking_id_trim ) ) {
						$shipment_labels .= $easypost_label['tracking_number'] . ',';
					} else {
						$shipment_labels = '';
					}
				}
				$shipment_source_data['shipment_id_cs'] = $shipment_labels . $shipment_id_cs;
			} else {
				$shipment_source_data['shipment_id_cs'] = $shipment_id_cs;
			}
			$shipment_source_data['shipping_service'] = $shipping_service;
		...
		if ( null !== $shipment_result && is_object( $shipment_result ) ) {
			$shipment_result_array = self::convert_shipment_result_obj_to_array( $shipment_result, $order_id );
			...
			$message = esc_attr( 'Unable to update tracking info.', 'woocommerce-shipment-tracking' );
	...
	public static function get_shipment_result( $shipment_source_data ) {
		EasypostWfTrackingFactory::init();
		...
		$wf_tracking                            = EasypostWfTrackingFactory::create( $shipment_source_obj );
