 ...
 */
abstract class EasypostWfTrackingAbstract {
	...
	private function load_tracking_data() {
		$this->tracking_data = EasypostWfTrackingUtil::load_tracking_data();
	}
	...
	private function get_tracking_link( $tracking_url, $shipment_id, $shipping_postcode ) {
		...
		$tracking_link = '';
		if ( '' !== $tracking_url ) {
			if ( stripos( $tracking_url, EasypostWfTrackingUtil::TAG_SHIPMENT_ID ) ) {
				$tracking_link = $tracking_url;
				$tracking_link = str_replace( EasypostWfTrackingUtil::TAG_SHIPMENT_ID, $shipment_id, $tracking_link );
			} else {
				$tracking_link = $tracking_url . $shipment_id;
			}

			if ( stripos( $tracking_link, EasypostWfTrackingUtil::TAG_SHIPPING_POST_CODE ) ) {
				$shipping_postcode = preg_replace( '/\s+/', '', $shipping_postcode );
				$tracking_link     = str_replace( EasypostWfTrackingUtil::TAG_SHIPPING_POST_CODE, $shipping_postcode, $tracking_link );
			}
		}
