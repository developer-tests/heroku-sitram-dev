        ...
        <ul class='woof_authors '>
            ...
            if (!isset($price2_filter_data['ranges']['options']) OR!is_array($price2_filter_data['ranges']['options'])) {
                ...
                foreach ($price2_filter_data['ranges']['options'] as $k => $value): $value = trim($value);
                    ...
                    ?>
                    <li class="woof_list">
                        <input type="radio" <?php if ($c == 0 AND $show_count): ?>disabled=""<?php endif; ?> class="woof_price_filter_radio" <?php checked($price2_filter_data['selected'], $k) ?>  name="woof_price_radio" id="woof_price_radio_<?php esc_attr_e($unique_id) ?>" value="<?php esc_html_e($k) ?>"  />
