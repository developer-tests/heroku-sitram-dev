    ...
    <?php if (!empty($terms) AND is_array($terms)): ?>
        <?php foreach ($terms as $term) : $inique_id = uniqid(); ?>
            ...
            <li class="woof_term_<?php esc_attr_e($term['term_id']) ?> <?php if ($hide_next_term_li): ?>woof_hidden_term<?php endif; ?>" <?php if ($this->settings['dispay_in_row'][$tax_slug] AND empty($term['childs'])): ?>style="display: inline-block;"<?php endif; ?>>
                <input type="checkbox" <?php if (!$count AND!in_array($term['slug'], $current_request) AND $show_count): ?>disabled=""<?php endif; ?> id="<?php esc_attr_e('woof_' . $term['term_id'] . '_' . $inique_id) ?>" class="woof_checkbox_term woof_checkbox_term_<?php esc_attr_e($term['term_id']) ?>" data-tax="<?php esc_attr_e($this->check_slug($tax_slug)) ?>" name="<?php esc_attr_e($term['slug']) ?>" data-term-id="<?php esc_attr_e($term['term_id']) ?>" value="<?php esc_attr_e($term['term_id']) ?>" <?php checked(in_array($term['slug'], $current_request)) ?> />
