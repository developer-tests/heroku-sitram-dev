...
if (isset(woof()->settings['by_rating']) AND woof()->settings['by_rating']['show']) {
    ...
    <div data-css-class="woof_by_rating_container" class="woof_by_rating_container woof_container">
        ...
        <div class="woof_container_inner <?php esc_attr_e($as_star ? "woof_star_selected" : "") ?>">
            ...
            <input type="hidden" value="<?php esc_html_e('Min rating: ', 'woocommerce-products-filter') ?><?php esc_html_e($selected) ?>" data-anchor="woof_n_<?php esc_attr_e("min_rating") ?>_<?php esc_attr_e($selected) ?>" />
        </div>
    </div>
    <?php
}
